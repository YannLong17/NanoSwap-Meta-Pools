...
from metapool.testing.configTestnet import ASSET1_ID, ASSET2_ID, USTEST_ID, METAPOOL_APP_ID, FEE_BPS, MIN_INCREMENT
...
# Create a Test Asset
if not USTEST_ID:
    USTEST_IS = newTestToken(amm_client.algod, creator_account)
        
ustest_asset = Asset(amm_client, USTEST_ID)
...
for asset in [asset1, asset2, lp_asset, ustest_asset]:
    if not amm_client.is_opted_into_asset(asset):
        print(creator_account.getAddress() + " not opted into asset " + asset.name)
        txn = get_payment_txn(amm_client.algod.suggested_params(), creator_account.getAddress(), creator_account.getAddress(), int(0), asset_id=asset.asset_id)
