...
def startup():
    ...
    amm_client = AlgofiAMMTestnetClient(algod_client=AlgodClient(ALGOD_TOKEN, ALGOD_ADDRESS), indexer_client=None, user_address=creator_account.getAddress())
...
def newTestToken(client: AlgofiAMMClient, creator: Account):
    """
    ...
    try:
        ...
        USTEST_ID= ptx["asset-index"]
        print(client.indexer.accounts(asset_id = USTEST_ID)['accounts']['created-assets'])
        return USTEST_ID
...
def update_metapool(algod_client: AlgodClient, creator: Account, metapool_app_id: int):
    ...
    txn = transaction.ApplicationUpdateTxn(creator.getAddress(), algod_client.suggested_params(), metapool_app_id, approval_program, clear_program)
    # sign, send, await 
    stxn = txn.sign(creator.getPrivateKey())
