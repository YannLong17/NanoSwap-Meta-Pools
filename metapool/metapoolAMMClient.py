...
from algofi_amm.utils import int_to_bytes, wait_for_confirmation, get_application_global_state, get_account_balances, get_payment_txn
...
class MetapoolAMMClient():
    ...
    def __init__(
        ...
        if metapoolAppID:
            ...
            self.metapool_lp_asset_id = getPoolTokenId(get_application_global_state(self.client.indexer, metapoolAppID))
    ...
    def fromMetapoolId(cls, client: AlgofiAMMClient, metapoolAppID: int):
        ...
        try:
            nanopool = client.get_pool(PoolType.NANOSWAP, appGlobalState[metapool_strings.nanopool_asset_1_id], appGlobalState[metapool_strings.nanopool_asset_2_id])
            return cls(
                client, 
                nanopool,
            ...
            raise RuntimeError(
            "Make sure the metapool app has been set up"
        )
    ...
    def setupMetapool(self, user: Account, feeBps: int, minIncrement: int) -> int:
        ...
        appGlobalState = get_application_global_state(self.client.indexer, self.metapool_application_id)
    ...
    def add_liquidity(self, user: Account, qA: int, qB: int) -> None:
        ...
        appCallTxn = transaction.ApplicationCallTxn(
            ...
            foreign_assets=[self.meta_asset_id, self.nanopool.lp_asset_id, self.metapool_lp_asset_id],
            sp=params,
        )
    ...
    def metaswap(self, user: Account, inTokenId: int, amount: int, outTokenId: int):
        ...
        if inTokenId == self.nanopool.asset1.asset_id or inTokenId == self.nanopool.asset2.asset_id:
        ...
        assert get_account_balances(self.client.indexer, user.getAddress())[inTokenId] > amount, "Not Enough Balance"
        ...
        appCallTxn = transaction.ApplicationNoOpTxn(
            ...
            index = self.metapool_application_id,
            app_args = app_args,
            foreign_apps = [self.nanopool.application_id, self.nanopool.manager_application_id],
            foreign_assets = assets,
    ...
    def get_zap_amount(self, asset_id, in_swap_amt):
        ...
        if asset_id == self.nanopool.asset1.asset_id:
            y = (in_swap_amt*self.nanopool.asset2_balance)/(self.nanopool.asset1_balance + self.nanopool.asset2_balance + in_swap_amt)
            # sign = 1
        elif asset_id == self.nanopool.asset2.asset_id:
            y = (in_swap_amt*self.nanopool.asset1_balance)/(self.nanopool.asset2_balance + self.nanopool.asset1_balance + in_swap_amt)
            # sign = -1
        ...
        if asset_id == self.nanopool.asset1.asset_id:
            ...
            a_b = (self.nanopool.asset1_balance - f_y.asset1_delta)/(self.nanopool.asset2_balance - f_y.asset2_delta)
            ...
            a_b = (self.nanopool.asset2_balance - f_y.asset2_delta)/(self.nanopool.asset1_balance - f_y.asset1_delta)
    ...
    def fundMetapool(self, user: Account, amount):
        ...
        """
        fundingTxn = get_payment_txn(self.client.algod.suggested_params(),user.getAddress(),self.metapool_address,amount)
    ...
    def optInToPoolToken(self, user: Account):
        ...
        optInTxn = transaction.AssetOptInTxn(
            sender=user.getAddress(), index=poolToken, sp=self.client.algod.suggested_params()
        )
    ...
    def metaswap_dryrun(self, user: Account, inTokenId: int, amount: int, outTokenId: int):
        ...
        signedInSwapTxn = inSwapTxn.sign(user.getPrivateKey())
        signedAppCallTxn = appCallTxn.sign(user.getPrivateKey())
        drr = transaction.create_dryrun(self.client.algod, [signedInSwapTxn, signedAppCallTxn])
        filename = "dryrun.msgp"
        with open(filename, "wb") as f:
            f.write(b64decode(msgpack_encode(drr)))

    def metaswap_unsafe(self, user: Account, inTokenId: int, amount: int, outTokenId: int):
